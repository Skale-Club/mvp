...
export function ConsultingStepsSection({ section, onCtaClick }: Props) {
  ...
  const ctaLabel = section.ctaButtonLabel || 'Schedule a Free Consultation';
  ...
  const practicalTitle = section.practicalBlockTitle || 'In Practice';
  const tagLabel = section.tagLabel || 'Consulting';
  const stepLabel = section.stepLabel || 'Step';
  const whatWeDoLabel = section.whatWeDoLabel || 'What We Do';
  const outcomeLabel = section.outcomeLabel || 'You Walk Away With';
  const practicalBlockSubtitle = section.practicalBlockSubtitle || 'How the work happens day to day';
  const nextStepLabel = section.nextStepLabel || 'Next Step';
  const nextStepText = section.nextStepText || 'Open schedule for new projects';
